#!/usr/bin/env bash
# DeepSeek Opus Round1: Apply choice-head → SFT approach (Doubao R7 breakthrough) to DeepSeek teacher
# Key innovation: DeepSeek has 14% teacher-GT disagreement → meaningful distillation signal
# Teacher hard labels converted to soft via label smoothing (eps=0.25)
set -euo pipefail
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_deepseek_opus_r1"
RUN_ROOT="$ROOT_DIR/runs/$RUN_ID"
mkdir -p "$RUN_ROOT/logs" "$RUN_ROOT/outputs" "$RUN_ROOT/artifacts"

DEEPSEEK_TEACHER_RAW="$BENCH_DIR/distill_runs/deepseek_v3_selective/runs/20260405_152319/artifacts/teacher_train_selective.jsonl"
DEEPSEEK_TEACHER_SOFT="$RUN_ROOT/artifacts/teacher_train_soft.jsonl"
...
MASTER_LOG="$RUN_ROOT/logs/opus_master.log"

{
  echo "[INFO] RUN_ID=$RUN_ID"
  echo "[INFO] Strategy: Choice-head distill (DeepSeek teacher) → GT SFT"
  echo "[INFO] Params: alpha_stage1=0.35, lr=0.00012, stage1_ep=1, stage2_ep=2"

  echo "[STEP 1] Convert DeepSeek hard labels to soft (label smoothing eps=0.25)"
  "$PY" "$ROOT_DIR/prepare_soft_labels.py" \
    --input "$DEEPSEEK_TEACHER_RAW" \
    --output "$DEEPSEEK_TEACHER_SOFT" \
    --smooth_eps 0.25
  ...
  "$PY" "$BENCH_DIR/distill_runs/shared/build_selective_distill_dataset.py" \
    ...
    --teacher_soft "$DEEPSEEK_TEACHER_SOFT" \
  ...
  echo "[STEP 3] Two-stage training (5 seeds)"
...
for p in params:
    ...
    # Stage1: choice-head distillation with DeepSeek teacher
    # Uses train_dental_choice_head_distill_doubao.py (teacher-agnostic, uses TeacherDist field)
    stage1 = [
    ...
    # Stage2: standard GT SFT, resuming from stage1 LoRA
    stage2 = [
...
summary = {
  ...
  'teacher': 'deepseek_v3',
  'approach': 'choice-head-distill → GT-SFT (opus)',
  'baseline': baseline,
  'previous_best_deepseek': prev_best,
  'n': len(vals),
...
(run_root / 'opus_results_latest.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
print('[OUT]', run_root / 'opus_results_latest.json')
for r in rows:
...
PY3
  ...
  echo "[DONE] DeepSeek Opus R1 finished"
