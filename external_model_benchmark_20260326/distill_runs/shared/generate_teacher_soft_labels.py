...
def call_openai_compatible(candidate, system_prompt, user_prompt, timeout_sec, max_tokens, max_retries, temperature):
    ...
    for attempt in range(max_retries + 1):
        try:
            resp = requests.post(candidate["base_url"], headers=headers, json=payload, timeout=timeout_sec)
...
def main():
    ...
    parser.add_argument("--existing_labels", required=True, help="已有teacher标签jsonl，需含TeacherAnswer")
    ...
    parser.add_argument("--request_interval_sec", type=float, default=0.6)
    ...
    with open(out_path, mode, encoding="utf-8") as wf:
        for i, item in enumerate(rows, start=1):
            ...
            first_vote = str(item.get("TeacherAnswer") or item.get("Answer") or "").strip().upper()
            if first_vote not in OPTION_LETTERS:
                first_vote = ""
            ...
            try:
                ...
                second_vote = extract_answer_letter(raw)
                votes = [v for v in [first_vote, second_vote] if v in OPTION_LETTERS]
                if not votes:
                    failed += 1
                    continue

                dist = build_dist(votes)
                majority = pick_majority(dist, fallback=votes[0])

                item2 = dict(item)
                item2["TeacherVoteBase"] = first_vote
                item2["TeacherVoteExtra"] = second_vote
                item2["TeacherVoteExtraRaw"] = raw
                item2["TeacherDist"] = dist
                item2["TeacherAnswer"] = majority
                item2["Answer"] = majority
