...
TEACHER_DATA="$RUN_ROOT/artifacts/teacher_train.jsonl"

MASTER_LOG="$RUN_ROOT/logs/grid_master.log"

{
  echo "[INFO] RUN_ID=$RUN_ID"
  echo "[STEP] generate teacher labels once (DeepSeek)"
  ...
  if [[ "$LINES" -le 0 ]]; then
    echo "[FATAL] teacher data is empty"
    exit 2
  fi

  echo "[STEP] run grid training"
  "$PY" - "$ROOT_DIR/grid_params.json" "$RUN_ROOT" "$PROJECT_ROOT" "$BASE_MODEL" "$TEACHER_DATA" "$VAL_DATA" "$TEST_DATA" "$PY" <<'PY2'
import json
import subprocess
import sys
...
for p in params:
    ...
    cmd = [
        py,
        str(project_root / 'train_dental_lora7.py'),
    ...
    print(f"[RUN] {name} alpha={p['alpha']} temp={p['temperature']}", flush=True)
    with log_path.open('w', encoding='utf-8') as lf:
        proc = subprocess.run(cmd, stdout=lf, stderr=subprocess.STDOUT)
    if proc.returncode != 0:
        print(f"[FAIL] {name} returncode={proc.returncode}", flush=True)
    else:
        print(f"[DONE] {name}", flush=True)
PY2

  "$PY" "$ROOT_DIR/summarize_grid_results.py" \
    --grid_root "$ROOT_DIR" \
    --run_id "$RUN_ID"
