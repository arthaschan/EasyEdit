...
RUN_ID="$(date +%Y%m%d_%H%M%S)_doubao_improve_r4"
...
PARAMS_JSON="$ROOT_DIR/grid_params_improve_round4.json"
MASTER_LOG="$RUN_ROOT/logs/improve_master.log"
...
for p in params:
    ...
    print(f"[RUN][improve-r4] {name}", flush=True)
    with log_path.open('w', encoding='utf-8') as lf:
        rc = subprocess.run(cmd, stdout=lf, stderr=subprocess.STDOUT).returncode
    if rc != 0:
        print(f"[FAIL][improve-r4] {name} rc={rc}", flush=True)
    else:
        print(f"[DONE][improve-r4] {name}", flush=True)
PY2
...
(run_root / 'improve_results_latest.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
lines = [
  '# Doubao Improve Round4 Results',
  '',
...
for r in ok:
    lines.append(
      f"| {rank} | {r['name']} | {r['learning_rate']:.5f} | {r['num_epochs']} | {r['rank']} | {r['lora_alpha']} | {r['seed']} | {r['accuracy']:.2f} | {r['accuracy']-baseline:+.2f} | {r['accuracy']-prev_best:+.2f} | ok |"
    )
    rank += 1
for r in rows:
    if r['status'] != 'ok':
      lines.append(
        f"| - | {r['name']} | {r['learning_rate']:.5f} | {r['num_epochs']} | {r['rank']} | {r['lora_alpha']} | {r['seed']} | - | - | - | failed |"
      )

(run_root / 'improve_results_latest.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
print('[OUT]', run_root / 'improve_results_latest.json')
print('[OUT]', run_root / 'improve_results_latest.md')
PY3

  echo "[DONE] doubao improve round4 finished"
