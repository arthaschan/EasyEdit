...
RUN_ID="$(date +%Y%m%d_%H%M%S)_doubao_improve_r3"
...
PARAMS_JSON="$ROOT_DIR/grid_params_improve_round3.json"
...
for p in params:
    ...
    print(f"[RUN][improve] {name}", flush=True)
    with log_path.open('w', encoding='utf-8') as lf:
        rc = subprocess.run(cmd, stdout=lf, stderr=subprocess.STDOUT).returncode
    if rc != 0:
        print(f"[FAIL][improve] {name} rc={rc}", flush=True)
    else:
        print(f"[DONE][improve] {name}", flush=True)
PY2
...
PY3
  ...
  echo "[DONE] doubao improve round3 finished"
