...
RUN_ID="$(date +%Y%m%d_%H%M%S)_doubao_soft_r5"
RUN_ROOT="$ROOT_DIR/runs/$RUN_ID"
mkdir -p "$RUN_ROOT/logs" "$RUN_ROOT/outputs"

TEACHER_DATA_BASE="$ROOT_DIR/artifacts/teacher_train.jsonl"
TEACHER_DATA_SOFT="$ROOT_DIR/artifacts/teacher_train_soft_r5.jsonl"
...
PARAMS_JSON="$ROOT_DIR/grid_params_soft_round5.json"
MASTER_LOG="$RUN_ROOT/logs/soft_master.log"

{
  echo "[INFO] RUN_ID=$RUN_ID"
  if [[ ! -f "$TEACHER_DATA_BASE" ]]; then
    echo "[ERROR] teacher_data missing: $TEACHER_DATA_BASE"
    exit 2
  fi

  echo "[STEP] generate soft labels"
  "$PY" "$BENCH_DIR/distill_runs/shared/generate_teacher_soft_labels.py" \
    --existing_labels "$TEACHER_DATA_BASE" \
    --candidate "$TEACHER_CANDIDATE" \
    --system_prompt "$SYSTEM_PROMPT" \
    --output "$TEACHER_DATA_SOFT" \
    --temperature 0.7 \
    --request_interval_sec 0.6 \
    --resume

  "$PY" - "$PARAMS_JSON" "$RUN_ROOT" "$PROJECT_ROOT" "$BASE_MODEL" "$TEACHER_DATA_SOFT" "$VAL_DATA" "$TEST_DATA" "$PY" <<'PY2'
import json
import subprocess
import sys
...
for p in params:
    ...
    print(f"[RUN][soft-r5] {name}", flush=True)
    with log_path.open('w', encoding='utf-8') as lf:
        rc = subprocess.run(cmd, stdout=lf, stderr=subprocess.STDOUT).returncode
    if rc != 0:
        print(f"[FAIL][soft-r5] {name} rc={rc}", flush=True)
    else:
        print(f"[DONE][soft-r5] {name}", flush=True)
PY2
...
(run_root / 'soft_results_latest.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
lines = [
...
for r in ok:
    lines.append(
      f"| {rank} | {r['name']} | {r['learning_rate']:.5f} | {r['num_epochs']} | {r['rank']} | {r['lora_alpha']} | {r['alpha']:.2f} | {r['seed']} | {r['accuracy']:.2f} | {r['accuracy']-baseline:+.2f} | {r['accuracy']-prev_best:+.2f} | ok |"
    )
    rank += 1
for r in rows:
    if r['status'] != 'ok':
      lines.append(
        f"| - | {r['name']} | {r['learning_rate']:.5f} | {r['num_epochs']} | {r['rank']} | {r['lora_alpha']} | {r['alpha']:.2f} | {r['seed']} | - | - | - | failed |"
      )

(run_root / 'soft_results_latest.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
print('[OUT]', run_root / 'soft_results_latest.json')
print('[OUT]', run_root / 'soft_results_latest.md')
PY3

  echo "[DONE] doubao soft round5 finished"
