...
RUN_ID="$(date +%Y%m%d_%H%M%S)_doubao_selmix_r6"
...
PARAMS_JSON="$ROOT_DIR/grid_params_selective_mix_round6.json"
...

{
  ...
  echo "[STEP] build selective mixed dataset"
...
lines = [
  '# Doubao Selective-Mix Round6 Results',
  '',
...
(run_root / 'selmix_results_latest.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
print('[OUT]', run_root / 'selmix_results_latest.json')
print('[OUT]', run_root / 'selmix_results_latest.md')
PY3

  echo "[DONE] doubao selective-mix round6 finished"
