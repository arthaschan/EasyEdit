...
RUN_ID="$(date +%Y%m%d_%H%M%S)_doubao_td_r3_mv5"
...
PARAMS_JSON="$ROOT_DIR/grid_params_two_stage_teacherdist_round3_mv5.json"
MASTER_LOG="$RUN_ROOT/logs/td_mv5_master.log"

{
  echo "[INFO] RUN_ID=$RUN_ID"

  if [[ ! -f "$TRAIN_DATA_BASE" ]]; then
    echo "[ERROR] missing base teacher data: $TRAIN_DATA_BASE"
    exit 2
  fi

  echo "[STEP] build mv5 soft labels"
  "$PY" "$BENCH_DIR/distill_runs/shared/generate_teacher_soft_labels_multivote.py" \
    --existing_labels "$TRAIN_DATA_BASE" \
    --candidate "$TEACHER_CANDIDATE" \
    --system_prompt "$SYSTEM_PROMPT" \
    --output "$TRAIN_DATA_SOFT" \
    --extra_votes 4 \
    --temperature 0.9 \
    --timeout_sec 25 \
    --max_retries 0 \
...
PY2
  ...
  "$PY" - "$PARAMS_JSON" "$RUN_ROOT" "$TRAIN_DATA_SOFT" <<'PY3'
import json
import math
import re
...
run_root = Path(sys.argv[2])
soft_data_path = Path(sys.argv[3])
baseline = 77.11
prev_best = 80.72
...
ent = []
peaks = []
for line in soft_data_path.read_text(encoding='utf-8', errors='ignore').splitlines():
    if not line.strip():
        continue
    r = json.loads(line)
    d = r.get('TeacherDist', {})
    vals = [float(d.get(k, 0.0)) for k in ['A', 'B', 'C', 'D', 'E']]
    s = sum(vals)
    if s <= 0:
        continue
    vals = [v / s for v in vals]
    ent.append(-sum(v * math.log(v + 1e-12) for v in vals))
...
summary = {
  ...
  'soft_entropy_mean': round(st.mean(ent), 4) if ent else None,
  'soft_peak_mean': round(st.mean(peaks), 4) if peaks else None,
  'soft_peak_ge_099_ratio': round(sum(1 for x in peaks if x >= 0.99) / len(peaks), 4) if peaks else None,
  'rows': rows,
}

(run_root / 'td_mv5_results_latest.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
lines = [
  '# Doubao Two-Stage TeacherDist Round3 (MV5) Results',
  '',
  f'- baseline_student_acc: {baseline:.2f}%',
  f'- previous_best_acc: {prev_best:.2f}%',
  f"- soft_entropy_mean: {summary['soft_entropy_mean']}",
  f"- soft_peak_mean: {summary['soft_peak_mean']}",
  f"- soft_peak_ge_099_ratio: {summary['soft_peak_ge_099_ratio']}",
  '',
...
(run_root / 'td_mv5_results_latest.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
print('[OUT]', run_root / 'td_mv5_results_latest.json')
print('[OUT]', run_root / 'td_mv5_results_latest.md')
PY3

  echo "[DONE] doubao two-stage teacherdist round3 mv5 finished"
