...
RUN_ID="$(date +%Y%m%d_%H%M%S)_doubao_headsft_r7"
...
PARAMS_JSON="$ROOT_DIR/grid_params_two_stage_head_then_sft_round7.json"
...
(run_root / 'headsft_results_latest.md').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
print('[OUT]', run_root / 'headsft_results_latest.json')
print('[OUT]', run_root / 'headsft_results_latest.md')
PY3

  echo "[DONE] doubao two-stage head-then-sft round7 finished"
