...
RUN_ID="$(date +%Y%m%d_%H%M%S)_round8_sft5_seeded"
...
PARAMS_JSON="$ROOT_DIR/grid_params_round8_sft5_seeded.json"

MASTER_LOG="$RUN_ROOT/logs/round8_sft5_seeded_master.log"

{
  echo "[INFO] RUN_ID=$RUN_ID"
  echo "[STEP] run seeded SFT x5"

  "$PY" - "$PARAMS_JSON" "$RUN_ROOT" "$PROJECT_ROOT" "$BASE_MODEL" "$TRAIN_DATA_GT" "$VAL_DATA" "$TEST_DATA" "$PY" <<'PY2'
import json
import subprocess
import sys
...
for p in params:
    ...
    print(f"[RUN][sft-seeded] {name} seed={p['seed']}", flush=True)
    with log_path.open('w', encoding='utf-8') as lf:
        rc = subprocess.run(cmd, stdout=lf, stderr=subprocess.STDOUT).returncode
    if rc != 0:
        print(f"[FAIL][sft-seeded] {name} rc={rc}", flush=True)
    else:
        print(f"[DONE][sft-seeded] {name}", flush=True)
PY2
...
(run_root / 'sft_seeded_summary.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
lines = [
    '# Round8 Seeded SFT-5 Summary',
    '',
    f"- run_root: {run_root}",
    f"- n: {summary['n']}",
...
if vals:
    lines += [
        ...
        '',
        '| Rank | Name | Seed | Accuracy(%) | Delta vs Baseline |',
        '|---:|---|---:|---:|---:|',
    ]
    for i, r in enumerate(ok, start=1):
        lines.append(f"| {i} | {r['name']} | {r['seed']} | {r['accuracy']:.2f} | {r['accuracy']-baseline:+.2f} |")
else:
    lines.append('- no valid runs')

(run_root / 'sft_seeded_summary.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
print('[OUT]', run_root / 'sft_seeded_summary.json')
print('[OUT]', run_root / 'sft_seeded_summary.md')
PY3

  echo "[DONE] round8 seeded sft finished"
