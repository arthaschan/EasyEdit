...
def main():
    ...
    for p in params:
        ...
        rows.append({
            ...
            "stage1_log": str(run_root / "logs" / f"stage1_{name}.log"),
    ...
    out_json = run_root / "two_stage_results_latest.json"
    out_md = run_root / "two_stage_results_latest.md"
    ...
    if best:
        ...
        lines.append("- If best still <= baseline, reduce alpha further (0.00-0.10) and raise GT anchor ratio.")
        lines.append("- If best > baseline, run narrow search around best with lr +/-5e-5.")
    else:
        lines.append("- No successful run found. Check logs first.")
