...

{
  ...
  "$PY" - "$ROOT_DIR/grid_params_round3.json" "$RUN_ROOT" "$PROJECT_ROOT" "$BASE_MODEL" "$TRAIN_DATA_GT" "$TEACHER_DATA_SELECTIVE" "$VAL_DATA" "$TEST_DATA" "$PY" <<'PY2'
import json
import subprocess
import sys
...
PY2
  ...
  "$PY" "$ROOT_DIR/shared/summarize_two_stage_results.py" \
    --run_root "$RUN_ROOT" \
    --params "$ROOT_DIR/grid_params_round3.json" \
    --baseline 77.11

  echo "[DONE] two-stage round3 finished"
} 2>&1 | tee "$MASTER_LOG"
