...
lines = []
lines.append('# 论文可用结果小结（Round-4 Narrow）')
lines.append('')
...
if best:
    ...
    lines.append('## 实验解释')
    lines.append('')
    lines.append('- 本轮为最优点邻域窄域搜索，用于验证增益稳定性与参数敏感区间。')
    lines.append('- 若最优继续提升，说明上一轮最优附近仍有可挖掘空间。')
else:
    lines.append('## 关键结论')
    lines.append('')
...
print('[DONE] round4 thesis summary and journal updated')
PY
